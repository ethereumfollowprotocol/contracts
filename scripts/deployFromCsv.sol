...
contract EFPScript is Script {
    ...
    function run() public {
        ...
        for (uint tokenId = 0; tokenId <= lastTokenId; tokenId++) {
            ...
            console.log("Appending %d records to EFP List NFT #%d", records.length, tokenId);
            ...
            console.log("EFP List NFT #%d    nonce: %d    numRecords: %s", tokenId, tokenId, numRecords);
            if (numRecords == 0) {
                continue;
            }
            DeletableListEntry[] memory recordsInRange = nonceArrayLists.getRecordsInRange(tokenId, 0, numRecords - 1);
    ...
    function readCSVFile(string memory filePath) public view returns (string memory) {
    ...
    function parseCsv(string memory csv) internal {
        ...
        for (uint i = 1; i < lines.length; i++) {  // Start from 1 to skip the header
            ...
            console.log('i=%d, require(efp_nft_token_id=%d >= %d=lastTokenId, "tokenIds are not monotonically increasing");', i, efp_nft_token_id, lastTokenId);
            ...
            console.log("LOADED EFP NFT #%d record #%d as %s", efp_nft_token_id, record_num, bytesToHexString(data));
    ...
    function parseAddress(string memory s) internal pure returns (address) {
        bytes memory b = bytes(s);
        uint160 result = 0;
        for (uint i = 2; i < 42; i++) { // start from 2 to skip the "0x"
            uint8 temp;
            if (uint8(b[i]) >= 97) { // 'a'
    ...
    function bytesCopy(bytes memory source, uint sourceStart, uint sourceLength, bytes memory destination, uint destinationStart) internal pure {
