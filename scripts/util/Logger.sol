...
library Logger {
    function formatListOp(
        ...
        s = string.concat(
            s,
            StringUtils.MAGENTA,
            StringUtils.byteToHexString(uint8(op.data[1]))
        );
        s = string.concat(
            s,
            StringUtils.CYAN,
            StringUtils.bytesToHexStringWithoutPrefix(
                BytesUtils.slice(op.data, 2, op.data.length - 2)
            ),
            StringUtils.ENDC
        );
        return s;
    ...
    function logListOps(
        uint start,
        uint end,
        mapping(uint => ListOp[]) storage listOpsMapping
    ) public {
