...
contract List {
    ...
    function appendRecord(
        ...
        DeletableListEntry memory newRecord = DeletableListEntry(
            false,
            ListRecord(version, recordType, data)
        );
    ...
    function deleteRecord(bytes32 recordHash) public {
        require(
            recordIndex[recordHash] > 0 &&
                !records[recordIndex[recordHash] - 1].deleted,
    ...
    function getRecordsInRange(
        uint fromIndex,
        uint toIndex
    ) public view returns (ListRecord[] memory) {
