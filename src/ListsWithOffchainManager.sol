...
contract ListsWithOffchainManager is BaseLists {
     ...
     * @param tokenId The ID of the token whose manager's authority needs validation.
     ...
     */
    function proveListManagerWithOffchainSignature(uint tokenId, address manager, bytes calldata signature) internal view returns (bool) {
        ...
        bytes memory message = makeSignatureMessage(tokenId, manager);
    ...
    function makeSignatureMessage(uint tokenId, address manager) internal pure returns (bytes memory) {
     ...
     * @param tokenId The ID of the token the manager is claiming authority over.
     ...
     */
    function claimListManagerWithOffchainSignature(uint tokenId, bytes calldata signature) external {
        require(proveListManagerWithOffchainSignature(tokenId, msg.sender, signature), "Invalid signature");
        managers[tokenId] = ListManager({
            isSet: true,
            managerAddress: msg.sender
        });
    }

    function getListManager(uint tokenId) external view returns (address) {
        return managers[tokenId].managerAddress;
    }
