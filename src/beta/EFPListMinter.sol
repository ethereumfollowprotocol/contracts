...
contract EFPListMinter {
    ...
    function mintWithListLocationOnL1AndSetAsDefaultList(
        uint nonceL1
    ) public payable {
    ...
    function mintToWithListLocationOnL1AndSetAsDefaultList(
        address to,
        uint nonceL1
    ) public payable {
        uint tokenId = registry.totalSupply();
    ...
    function mintWithListLocationOnL2AndSetAsDefaultList(
        uint chainId,
        address addressL2,
        uint nonceL2
    ) public payable {
    ...
    function _setDefaultListForAccount(address to, uint tokenId) internal {
        accountMetadata.setValueForAddress(
            to,
            'efp.list.primary',
    ...
    function _setListLocationL1(
        ...
        listMetadata.setValue(
            tokenId,
            'efp.list.location',
            abi.encodePacked(version, listLocationType, addr, nonce)
        );
    }

    function _setListLocationL2(
        uint tokenId,
        uint chainId,
        address addr,
        uint nonce
