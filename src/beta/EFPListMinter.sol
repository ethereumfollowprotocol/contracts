...
contract EFPListMinter {
    ...
    constructor(address _registryAddress, address _accountMetadataAddress, address _listMetadataAddress, address _listsAddressL1) {
    ...
    function mintToWithListLocationOnL2AndSetAsDefaultList(address to, uint chainId, address addressL2, uint nonceL2) public payable {
        uint tokenId = registry.totalSupply();
        registry.mintTo{
            value: msg.value
        }(to);
    ...
    function _setListLocationL2(uint tokenId, uint chainId, address addr, uint nonce) internal {
        ...
        listMetadata.setValue(tokenId, "efp.list.location", abi.encodePacked(version, listLocationType, chainId, addr, nonce));
    }
}
